{
  ...
  in {
    devShells.x86_64-linux.default =
      ...
      pkgs.mkShell {
        buildInputs = [
          python python.pkgs.pip python.pkgs.setuptools python.pkgs.wheel
    ...
    devShells.x86_64-linux.java21shell =
      let pkgs = unstablePkgs; in
      pkgs.mkShell {
        buildInputs = [ pkgs.jdk21 pkgs.maven pkgs.gradle pkgs.git pkgs.curl pkgs.jq ];
        shellHook = ''echo "Java 21 Shell"; java --version'';
    ...
    devShells.x86_64-linux.gptshell =
      ...
      pkgs.mkShell {
        buildInputs = [
          pkgs.playwright-driver.browsers pkgs.python312 pkgs.python312Packages.pip
          pkgs.python312Packages.setuptools pkgs.python312Packages.wheel
          pkgs.git pkgs.curl pkgs.wget pkgs.jq
        ];
        shellHook = ''
    ...
    devShells.x86_64-linux.agentshell =
      let pkgs = unstablePkgs; python = pkgs.python312; in
      pkgs.mkShell {
        buildInputs = [
          python python.pkgs.pip python.pkgs.setuptools python.pkgs.wheel
          python.pkgs.ipython python.pkgs.black python.pkgs.pytest
          python.pkgs.requests python.pkgs.virtualenv
          pkgs.nodejs_20 pkgs.openjdk17 pkgs.git pkgs.curl pkgs.jq
        ];
        shellHook = ''
